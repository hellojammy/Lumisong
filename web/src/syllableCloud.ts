...
import { fadeFactorAtPlayhead } from './fadeFactor.ts';
...
export class SyllableCloud {
  ...
  private finishedHold = false; // 023：播完定格全图，updateEffects 不再按时间轴渐隐
  // 020 特效状态
  ...
  private fadeFactor(i: number): number {
    return fadeFactorAtPlayhead(this.nowCache, this.ts[i], this.fxFade, this.finishedHold);
  }
  ...
  setGuidesVisible(visible: boolean): void {
    if (this.guidesVisible === visible) return;
    this.guidesVisible = visible;
    if (visible) {
      if (this.labels.size === 0) this.buildLabels();
      else this.applyLabelsVisibility(true);
    } else {
      this.applyLabelsVisibility(false);
    }
  }
  ...
  /** 谢幕结束后的幽灵全图（nowCache<0），供形态切换继承 */
  isGhostReplayState(): boolean {
    return this.nowCache < 0;
  }

  /** 023：播完谢幕后的形态重建——继承幽灵全图（暗但全在） */
  syncGhostState(): void {
    this.cancelFinale();
    this.active.clear();
    this.hasFocus = false;
  ...
  /** 023：播完后的形态重建同步。保持全图可见，不把结束态重新解释成渐隐推进。 */
  syncFinished(now: number): void {
    this.cancelFinale();
    this.active.clear();
    this.hasFocus = false;
    this.finishedHold = true;
    this.nowCache = now;
    const fade = this.fxFade;
    this.fxFade = false;
    this.resyncPlayed();
    this.updateLineProgress(now);
    this.writeLineColors();
    this.writeLabelsOpacity();
    this.fxFade = fade;
  }
  ...
  private applyLabelsVisibility(visible: boolean): void {
    if (this.batchedText) this.batchedText.visible = visible;
    for (const { text } of this.labels.values()) {
      (text as unknown as THREE.Object3D).visible = visible;
    }
  }
