...
const NOISY_FLATNESS = 0.45; // 归一化 flatness 超过此值 → 噪声性音节，用多面晶核
...
/** 内核材质（005 光体）：漫反射归零，颜色 100% 走逐实例 emissive */
function coreMaterial(flatShading: boolean): THREE.MeshStandardMaterial {
...
export class SyllableCloud {
  ...
  private shell: THREE.InstancedMesh | null = null; // 仅 orb 形态
  ...
  constructor(data: SyllablesJson, form: FormKey = 'orb', capacity = 0) {
    ...
    // —— 015 形态分支：路由函数把每个音节指到所属实例网格 ——
    let routeNext: (i: number) => CoreRef;
    if (capacity > 0) {
      // 022 录音态浮现：按容量预分配出现时刻/渐入时长（append 时逐颗写入，-1=未出现）
      this.recAppearWall = new Float32Array(cap).fill(-1);
      this.recFillDur = new Float32Array(cap).fill(REC_FILL_DEFAULT);
      // 022 录音模式：单核 orb，预分配 capacity 个槽，初始只渲染 n 个；append 填后续槽。
      const shell = new THREE.InstancedMesh(
        new THREE.SphereGeometry(1, 32, 20),
        new THREE.MeshPhysicalMaterial({
          transparent: true, opacity: 0.16, roughness: 0.12, metalness: 0.0,
      ...
      shell.count = n; // 仅渲染已生成的音节
      this.shell = shell;
      const core = new InstancedUniformsMesh(
        new THREE.SphereGeometry(0.72, 28, 18), coreMaterial(false), capacity,
      );
      core.count = n;
      this.instMeshes.push(core);
      routeNext = (i) => ({ mesh: core, idx: i });
      this.group.add(core, shell);
    } else if (form === 'orb') {
      // 形态分化：按音调性把内核分到两个实例网格（数据通道：flatness → 形态）
      const noisy = syllables.map((s) => norm(s.flatness, meta.ranges.flatness) > NOISY_FLATNESS);
      const nFacet = noisy.filter(Boolean).length;

      // 外层玻璃壳（uupm 系玻璃拟态）：着色玻璃 + 环境反光，包住发光核
      const shell = new THREE.InstancedMesh(
        new THREE.SphereGeometry(1, 32, 20),
        new THREE.MeshPhysicalMaterial({
          transparent: true,
          ...
          roughness: 0.12,
          metalness: 0.0,
          envMapIntensity: 0.5, // ⚠️ 高光-拖尾耦合：峰值须 < TrailPass 阈值（按形态 3.6–4.4，见 postfx TRAIL_BY_FORM）
          depthWrite: false,
        }),
        n,
      );
      ...
      // 注意：emissiveIntensity 不是独立 shader uniform（CPU 端折入 emissive 颜色），
      // 逐实例发光只能通过 setUniformAt('emissive', i, 颜色×强度) 实现。
      ...
      const coreFacet = new InstancedUniformsMesh(
        new THREE.IcosahedronGeometry(0.8, 0), // 纯发光下晶面靠多边形剪影区分，故偏大
        coreMaterial(true),
        Math.max(nFacet, 1),
      );
      this.instMeshes.push(coreSmooth, coreFacet);
      let si = 0;
      let fi = 0;
      routeNext = (i) => (noisy[i]
        ? { mesh: coreFacet, idx: fi++ }
        : { mesh: coreSmooth, idx: si++ });
      this.group.add(coreSmooth, coreFacet, shell);
    } else {
      // 单核形态：spire 六棱光针（长度=时长）/ ripple 水平光环（半径=响度）/
      // gem 八面体碎钻 / planet 球核（光环为第二网格）
      const geo = form === 'spire' ? new THREE.CylinderGeometry(0.05, 0.05, 1, 6)
        : form === 'ripple' ? new THREE.TorusGeometry(1, 0.07, 8, 48)
        : form === 'gem' ? new THREE.OctahedronGeometry(0.8, 0)
        : new THREE.SphereGeometry(0.85, 24, 16); // planet 核
      const m = new InstancedUniformsMesh(geo, coreMaterial(form === 'gem'), n);
      this.instMeshes.push(m);
      routeNext = (i) => ({ mesh: m, idx: i });
      this.group.add(m);
      if (form === 'planet') {
        // 018 星环：倾斜光环共享矩阵流，发光弱于球核
        this.ring = new InstancedUniformsMesh(
          new THREE.TorusGeometry(1.7, 0.05, 6, 40), coreMaterial(false), n,
        );
        this.instMeshes.push(this.ring);
  ...
  appendSyllables(batch: SyllableData[], meta: SyllablesJson['meta']): void {
    ...
    if (this.count > 0) {
      this.recFocus.copy(this.positions[this.count - 1]);
  ...
  updateFlare(now: number): void {
    ...
    if (this.hasFocus) this.focus.copy(this.positions[focusIdx]);
  ...
  /** 013/018/020：玻璃壳着色随填充进度渐变 × 渐隐（仅 orb 形态有壳） */
